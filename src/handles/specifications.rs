use crate::handles::get_spec::get_spec::GetSpec;
use crate::models::specifications::FullSpec;

use axum::{response::{IntoResponse, Json}};

pub async fn get_full_spec() -> impl IntoResponse {
    let mut get_spec = GetSpec::new().unwrap();
    let device = get_spec.device();
    let cpu = get_spec.cpu();
    let ram = get_spec.ram();
    let storage = get_spec.storage();
    let gpu = get_spec.gpu();

    let spec = Json(FullSpec { device, cpu, ram, storage, gpu });

    spec
}